...
// DecryptSbox is the 256-byte RC4 S-box used to decrypt client→server packets.
//
// Origin: BlowfishDecryptKey[2:258] from the reference Go implementation (main.go).
// The first two bytes of BlowfishDecryptKey are the initial i/j state (both 0x00),
// followed by 256 bytes that form the S-box, followed by 2 padding bytes never accessed.
//
// IMPORTANT: This key is separate from WelcomeKey (ConnectionClient payload).
// WelcomeKey is a GameGuard challenge sent to the client; it is NOT used for cipher.
// DecryptSbox is the actual cipher key, hardcoded in both client and server.
//
// Usage: create a fresh NewRC4(DecryptSbox) for every received packet (per-packet reset).
