// Package send contains outgoing (Server→Client) packet encoders for the Login Server.
package send

import "r2server/internal/network"

// WelcomeKey is the hardcoded 198-byte payload sent in opcode 1103 (ConnectionClient).
//
// This is a GameGuard authentication challenge: a 16-byte auth block encrypted with
// Blowfish (key "y].ZdaXlMls"), plus additional session data. The value here is a
// static snapshot used by the reference C# emulator (AuthorizationFactory.cs).
//
// A private server running without GameGuard will accept any valid-format challenge;
// the client skips the anti-cheat verification when the GameGuard DLL is absent.
var WelcomeKey = [198]byte{
	,
}

// ConnectionClient is opcode 1103 — the first packet the server sends on any connection.
// Payload is the raw WelcomeKey bytes.
type ConnectionClient struct{}

func (p *ConnectionClient) Encode() []byte {
	w := network.NewWriter()
	w.WriteBytes(WelcomeKey[:])
	return w.Bytes()
}
